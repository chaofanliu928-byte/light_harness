...
- `scope='all'` 时读全部 13 行;`scope='changed:...'`(预留)时只读端点列命中给定文件的触点行。
...
- 但本表另有体检来源行 **TP-09~12**(来源=「体检YYYY-MM-DD」)无 §8 对应,**不要求计数相等**。
- 判 TP-13 时**只查"§8 的每条都有本表行覆盖",不可反向拿本表行数 == §8 条数当判据**对 TP-09~12 误报 🔴(它们本就无 §8 对应,是体检摸到的散落触点)。
...
- **报告分层(DriftReport)**:出现任一 🔴/⚠️ → 返回 `DriftReport`:
  ...
  - **✅ 只进 `alignedCount` 折叠计数,不逐条**(避免 13 行 ✅ 刷屏)。
...
- **全一致**(13 触点全 ✅ 无 🔴/⚠️)→ `AllAligned` → 调度者一句"触点全一致"。
