...
| 裁决 | 动作 |
|------|------|
| **上架** | 写书架对应格(住址按下方路由表)+ **同批登记**(references/ → 目录卡 references/README.md 加行;decisions/ → 文件名即登记)+ 台账指针区加 `- -> 路径 — 一句话` |
...
**清账步必含兜底一问**:本会话有无该暂存未暂存的内容?(机器测不出"没写的东西",此问是在场兜底;归档件保全原文可考古)
...
1. 行数:`wc -l < docs/active/handoff.md` ≤ 80。超限砍序 = **先砍散文(关键上下文),指针与声明字段不砍**;散文砍尽仍超 → 允许超限,stderr 声明超限原因
2. 正文无 `[待填]` 残留(新建台账除外)
3. 文件路径真实存在;没有敏感信息
