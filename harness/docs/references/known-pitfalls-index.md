...
| drift-scout.md 硬编码「13 触点」计数随注册表增行变 stale:C 加 TP-14/15 后注册表 15 行,illustrative 计数过时(检测本就 count-agnostic L85-87,功能不受影响)。**✅ 2026-06-18 已修**:drift-scout.md 7 处「13 触点/行」→ count-agnostic(全部触点/动态行数)+ L68/L69 TP-13 护栏「TP-09~12」泛化为「体检来源行」,行为零改。**注**:`drift-detection-design.md` / `2026-06-16-drift-detection.md` plan 的「13」是设计期历史快照,不改(非操作件)。 | ✅已关闭(2026-06-18 count-agnostic 批) | 是 | `.claude/agents/drift-scout.md`(已修)+ audit `audit-2026-06-18-002538-drift-scout-count-agnostic.md`(exempt)+ 用户拍板 | ⚪ |
