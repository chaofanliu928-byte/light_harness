...
| 括号字段 | `(理由: <reason>)` 整体必出现 |
| reason 校验 | 非空非全空白,至少 1 个非空白字符(POSIX `\S` 匹配),与字段 1 同规则 |
| hook grep regex | `## meta-cross-ref: skipped\(理由: ([^)]+)\)`(POSIX ERE) |
| 覆盖语义 | 每次新 cross-ref hook 报错时调度者覆盖,不累积 |
| 不归档 | handoff mutable |
| 与字段 1 关系 | **独立字段**,marker 不同;用户在 handoff 写 `## meta-review: skipped` **不**让 cross-ref hook 放行;反之亦然 |
| 半角括号 | 与字段 1 同规则(必半角 `(` `)`) |
| spec 锚点 | P0.9.3 spec §3.1 cross-ref hook 伪码注释 |
| contract | 暂未入 contracts-locked.md(P0.9.3 第一个 trial 新增字段,后续若稳定纳入 C3 扩展) |

### 5.4 三字段共存约束(原"两字段共存约束"扩展 — audit D4-F1 修订)
