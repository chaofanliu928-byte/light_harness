...
10. 活跃任务索引机读表头 token + section 标题字面(T1↔T3 结构双写对):`handoff-template.md` 机读表头行 `活跃任务: N(进行中 X / 挂起 Y)` 整条版式逐字符(含空格/斜杠/括号)**+** section 标题字面 `## 活跃任务索引` ↔ `check-handoff.sh` `--reconcile` 表头文法 ERE 硬编码(`^活跃任务: [0-9]+\(进行中 [0-9]+ / 挂起 [0-9]+\)$`)+ 核② awk 的 `^## 活跃任务索引` 开窗硬编码(两处硬编码副本)。**文档上游、代码派生**:改一处(T1 模板的表头版式或 section 标题)必同批改另一处(T3 的 ERE/awk 硬编码);审查时触点完整性维比对逐字一致(含版式空格/斜杠/括号 + section 标题字面)。同 promotion GRAMMAR「三处同名同文法」先例(本设计 T2 走 `!cat` 运行时注入,故静态双写对是 T1↔T3 两处)。
