...
**Step 2 ★ 设计层到手边(✅ 上游段 ①②③a③b 完成 2026-06-16;✅ C 下游实现+收口完成 2026-06-17 audit pass merge-ready)= 漂移检测 + 触点完整性 + 设计背景到手边**:契约/模块地图/why/坑 在写代码·重构·调试场景可靠到手边(含漂移腐检测)。用户「先派侦察员摸 territory 再分头查」拆出执行序 **①体检漏改修复 → ②B 已知坑索引 → ③a 触点机读注册表(地基)→ ③b 漂移检测机制**;A+B 上游(harness 自身漂移检测/触点完整性),C(下游设计层导航)押后(上游理清后再设计)。**边界厘清(知识/偏好/规则)随 Step 2 真案例解,不单独前置**。
...
- **C 设计层到手边(下游)✅ 实现+收口完成**(2026-06-17,branch `design-context-delivery`,spec `2026-06-17-design-context-delivery-design.md` commit 566d85f):机读·设计背景地图(docs/governance,按业务模块,套 ③a 范式只指不抄)+ fork 侦察员 `design-context-scout`(克隆 ③b drift-scout 形态:pull / 读地图两跳[file→模块→住址]/ 按 scenario[write/debug/refactor]取片 / 消化 briefing / 只读不写)+ 下游迁移指南 `design-context-migration.md`(11 类必备内容清单 + 搬进格式三步)+ 保鲜复用 ③b drift-scout。设计链:brainstorming(**完整覆盖=代码 SSoT + 外化"代码答不出的"tacit 层** / 按内容切非按场景切 / 复用现有家不新建 wiki / 3 诚实赌注[下游真写料·pull 够·模块边界清])→ designer → 3 自检 → **design-review review-scout dogfood 第 4 次**(2🔴[迁移指南放不分发的 README + refactor[P0]掩盖用户初押后]+8🟡)→ 2 轮 designer 修 → round-2 复审 **0🔴 锁定**。
  - **carry-forward 🟡 ✅ 已落地**:setup.sh 地图 `basename skip` 与活文件守卫 cp **成对**已实现(T8);收口 触点完整性维 + setup.sh 静态核双验、`bash -n` clean、drift-scout TP-09 循环分发类不误报。
  - **✅ 实现+收口**(2026-06-17):writing-plans(规划决策 P-1 洞②`// WHY:` scattered-only 落 implementation-rules / P-2 DESIGN_TEMPLATE §1.7+§5.1 理由列)→ 8 任务 subagent-driven(全 spec-compliant + 跨文件一致 consistent)→ fixture 红线 5 案验收(2 红线✓:有料递对 / 料缺⚠️不编造)→ finishing:audit `audit-2026-06-17-230642-design-context-delivery.md` **verdict=pass**(8 covers:scout/map/migration/finishing-rules/touchpoint-registry/implementation-rules/setup.sh/DESIGN_TEMPLATE)+ drift-scout 15 触点全✅ + 方向评估 4 维 pass;收口 2🟡(示意行入注释 / missingKinds accept-with-rationale);decision-trail 2026-06-17 登记。**merge-ready**。
  - **实现期跟进(backlog)**:drift-scout「13 触点」计数随 TP-14/15 增至 15 行变 stale(检测 count-agnostic 不受影响)→ **单独一批 count-agnostic 化**(用户 2026-06-17 拍板,已登 known-pitfalls-index);C 机制稳定性(scout 误匹配率/编造率/下游写作率)挂 **meta-L4 实战观察**推真实项目(自仓库 dogfood 审不到);边界厘清(知识/偏好/规则)随真案例解。
  - **meta-L4 第 4 次正向数据点**:scout 推维**未退化**(保留地板2 + skipped 完整性留痕 + 现推 3 个 spec 专属维[克隆忠实性/触点完整性/spec_gap_masking]each `why_this_time` 引原文锚 + rubric_mode=template 正确判);**独立 design-review 逮到 system-design 自检漏的 🔴(README 不分发下游)——做审分离(公设1)实证有效,非仪式**。
