...
**Step 2 ★ 设计层到手边(✅ 上游段 ①②③a③b 完成 2026-06-16;C 下游设计锁定 2026-06-17、待 writing-plans)= 漂移检测 + 触点完整性 + 设计背景到手边**:契约/模块地图/why/坑 在写代码·重构·调试场景可靠到手边(含漂移腐检测)。用户「先派侦察员摸 territory 再分头查」拆出执行序 **①体检漏改修复 → ②B 已知坑索引 → ③a 触点机读注册表(地基)→ ③b 漂移检测机制**;A+B 上游(harness 自身漂移检测/触点完整性),C(下游设计层导航)押后(上游理清后再设计)。**边界厘清(知识/偏好/规则)随 Step 2 真案例解,不单独前置**。
...
- **C 设计层到手边(下游)设计锁定**(2026-06-17,branch `design-context-delivery`,spec `2026-06-17-design-context-delivery-design.md` commit 566d85f):机读·设计背景地图(docs/governance,按业务模块,套 ③a 范式只指不抄)+ fork 侦察员 `design-context-scout`(克隆 ③b drift-scout 形态:pull / 读地图两跳[file→模块→住址]/ 按 scenario[write/debug/refactor]取片 / 消化 briefing / 只读不写)+ 下游迁移指南 `design-context-migration.md`(11 类必备内容清单 + 搬进格式三步)+ 保鲜复用 ③b drift-scout。设计链:brainstorming(**完整覆盖=代码 SSoT + 外化"代码答不出的"tacit 层** / 按内容切非按场景切 / 复用现有家不新建 wiki / 3 诚实赌注[下游真写料·pull 够·模块边界清])→ designer → 3 自检 → **design-review review-scout dogfood 第 4 次**(2🔴[迁移指南放不分发的 README + refactor[P0]掩盖用户初押后]+8🟡)→ 2 轮 designer 修 → round-2 复审 **0🔴 锁定**。
  - **carry-forward 🟡(钉 writing-plans)**:setup.sh 地图排除出 governance `*.md` 无条件循环的 `basename skip` 须与活文件守卫 cp(`if [ ! -f ]`,同 handoff.md)**成对**(漏 skip=重装覆盖下游已填地图 / 漏守卫=地图不分发空转);spec §6.2 已列收口红线双查。
  - **下一步**:writing-plans(契约前置)→ subagent 实现 → finishing(audit covers 候选:`design-context-scout.md`/`design-context-map.md`/`design-context-migration.md`/`finishing-rules.md`/`touchpoint-registry.md`/`setup.sh`[+`DESIGN_TEMPLATE.md` 若改洞①业务规则段])。
