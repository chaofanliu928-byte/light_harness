...
2. **[P0] design ↔ finishing 互引完整检测**(D4 修复):
   ...
   - 用户在 finishing-rules.md L38 删除"design-rules.md \"轻量级\"判定"引用 → PAIRS 第 6 条 `design-rules.md|**轻量级**` 检 design L28 规模判断表内 markdown 加粗形式声明;若 design 改概念名(如把 `**轻量级**` 改 `**小改动**`)→ hook 报警
...
PAIRS=(
    ...
    'docs/governance/design-rules.md|**轻量级**'                          # 新 6(检 design 规模判断表内"轻量级"概念形式声明;finishing L38 引用)
)
```

注:
- 第 5 条 anchor `## 反模式约束` 是 finishing-rules.md L24 内部段标题,被 design L28 + L45 间接引用;若 finishing 删段,hook 报警
- 第 6 条 anchor `**轻量级**` 是 design-rules.md L28 规模判断表 markdown 加粗的形式概念声明(全文唯一一处);被 finishing L38 `"轻量级"判定` 间接引用;若 design 改概念名(如把 `**轻量级**` 改 `**小改动**`),hook 报警 + finishing L38 引用悬空(预期行为,提示同步)
- **修正记录**(spec 写时错):本条原写 `"轻量级"判定`(以为存在于 design-rules.md);实际该字面**仅**存在于 finishing-rules.md L38(反向引用),design-rules.md 内无此字面 — implementer 在 Step 2.4 grep 校验时暴露;改用 design L28 形式声明 `**轻量级**`(留痕详 §9.4 #25 第四次错)
...
| **C10** | PAIRS 第 6 条 `**轻量级**` 改名 | design 把 `**轻量级**` 改 `**小改动**` | hook 报警 + finishing L38 引用悬空 — 预期行为(用户必须同步改 finishing 或 PAIRS) |
...
| 4 | audit covers 字段写 `<root>/CLAUDE.md` → 命中 M3 改动 → 放行 | meta-L1 | hook exit 0(无 handoff skip 时);**实现期 handoff skip 兜底**下 exit 0 也成立(skip 字段直接放行,不到比对层) |
| 5 | audit covers 字段写 `CLAUDE.md` → 仅命中 M4,M3 仍未 cover → 阻断 | meta-L1 | hook exit 2 + stderr sentinel 提示(**无 handoff skip 时**);实现期有 `## meta-review: skipped` 字段时实际 exit 0,验证改用 grep stderr 含 `<root>/CLAUDE.md` 而非 exit code(详 §9.4 #26)|
| 6 | cross-ref hook PAIRS 第 5 条:删 finishing-rules.md L24 `## 反模式约束` 段 → hook 报警 | meta-L1 | check-meta-cross-ref exit 2 + 列具体缺失(**无 handoff skip 时**);实现期有 `## meta-cross-ref: skipped` 字段时实际 exit 0,验证改用 grep stderr 含 anchor 缺失提示而非 exit code(详 §9.4 #26)|
| 7 | cross-ref hook PAIRS 第 6 条:删 design-rules.md 内 `**轻量级**` 字面 → hook 报警 | meta-L1 | check-meta-cross-ref exit 2 + 列具体缺失(**无 handoff skip 时**);同场景 6 handoff skip 副作用(详 §9.4 #26)|
| 8 | R1 段 git -C 失败 fixture(模拟 .git 损坏)→ stderr warning + 主扫继续 | meta-L1 | hook exit 0 + stderr `⚠️ repo 根 git -C 调用失败`(**理论预期**);实现期 fixture 走 R2 路径(`.git` 不存在分支)替代,R1 真实路径继承 P0.9.3 §9.4 #13 dead path |
...
24. **PAIRS 第 5/6 条选择的局限性**:第 5 条用 anchor 段标题 `## 反模式约束`(被引方 anchor)— 若 finishing 把段标题改名(如 `## 反模式列表`),hook 报警但实际 design 引用文字未必同步改;第 6 条用被引方形式声明 `**轻量级**`(design L28 markdown 加粗 — 全文唯一 site),同样限制(若 design 改 `**轻量级**` 为 `**小改动**`,hook 报警但 finishing L38 引用文字 `"轻量级"判定` 未必同步改)。**接受**:PAIRS 是 grep 字面比对,不是语义比对(同 P0.9.3 #5)
25. **审查过程留痕(4 错 + 1 纠正,共 5 个留痕点)**(`feedback_spec_gap_masking` 元数据点):
   ...
   - **第四次错**(本 trial spec 写 §3.1 PAIRS 第 6 条 anchor 时):写 `"轻量级"判定` 时未 grep 验证字面是否在 design-rules.md 内;实际该字面**仅**存在于 finishing-rules.md L38(反向引用),design-rules.md 形式声明在 L28 用 markdown 加粗 `**轻量级**`;**implementer 在 Task 2 Step 2.4 grep 校验时暴露**;改用 `**轻量级**`(L28 唯一声明)
   - **第五次错**(第四次错的 correction commit `0189599` 自身 sweep 不全):commit 只更新 spec §3.1 PAIRS code block + §6.1 scenario 7 + §9.4 #25 + plan §2.1/2.4/2.6;**漏** spec §1.2 core scenario 2 / §5.1 C10 表 / §9.4 #24 + plan §2.8 commit msg template / §4.2 prepend / §5.2 prepend 共 6 处 stale `"轻量级"判定`;**Task 5 后 final code reviewer 暴露**;commit `785f6a7` 扫除 6 处
   - **教训**:hook 实现 trial(P0.9.3 第一个 + 本 trial)涉及大量字面 grep + anchor 比对工作,审查者 + 调度者 + 自审 + spec 写者都容易**视觉跳过间接引用 / 不验证 anchor 字面存在性 / 修复时只 sweep 命中点不全文扫**;后续 trial 涉及 anchor / PAIRS 比对时必须:
     1. 写 PAIRS 时**当场** `grep -F '<anchor>' <file>` 验证 anchor 字面在目标文件存在(spec 写时,而非 implementer 阶段才暴露)
     2. 区分**形式声明**(被引方权威定义,如 `**轻量级**` L28 表格)vs **引用文字**(引用方文字,如 `"轻量级"判定` finishing L38);anchor 应在被引方的形式声明 site
     3. 区分**直接引用 vs 间接引用**;PAIRS 应覆盖**互引方向 + 形式声明 site**两者
     4. **修字面错时全文 grep 扫所有 stale references**(scope = **全仓库** `harness/` 整体,**包含跨 trial 上游文件**: 上游 spec / decision narrative body / commit msg template / 引用方文字镜像 — 不限本 trial 文件);而非只改命中点 — 否则 stale ref 会通过 final reviewer 或后续 audit 暴露(第五次错教训 + meta-review 第六次错 = scope 仍局限本 trial 文件、漏上游 P0.9.3 第一个 trial spec L23 + decision §不做 L141 同字面 stale ref,由 audit 2026-04-30 challenger 3 抓出后修复)
   - 留痕作为 P0.9.3 trial 序列的元数据点(hook 实现 trial 的高 finding 密度部分来自这种"视觉跳过 / 字面未验证 / 修复 sweep 不全 / sweep scope 仍过窄"模式)

26. **实现期 inline 验证受 handoff skip 字段干扰**(audit 2026-04-30 challenger 3 F3.4 + challenger 2 数据 2):
   - 现象:Pre-Task Step 0 写入 `## meta-review: skipped` + `## meta-cross-ref: skipped` 字段后,场景 5/6/7 的 hook 实际 exit 0(skip 字段直接放行),与 §6.1 表格预期 exit 2 不一致
   - 表征:plan §1.10/1.11 + §2.5 注释里发现且 documented(用 grep stderr 替代 exit code 校验),但**未回写 spec §6.1 预期列**;原 spec 表格预期与实现期实际行为分歧
   - 影响:meta-L1 inline 验证的"全 8 场景实跑"声明在实施期需附加条件解读;读 spec 的人可能把"hook exit 2"当成测量目标
   - 处理:本 audit revision 把场景 5/6/7 预期列改为"hook exit 2(无 handoff skip 时)/ exit 0 + stderr 含 violations(实现期有 skip 时)"双态描述;场景 8 同步标"理论预期 vs 实现期 R2 替代"
   - **教训扩 第 5 条**(spec §9.4 #25 教训扩展):**测试场景预期列必须标注"是否依赖外部状态"**(如 handoff 字段、git 工作树状态)— 否则预期与实际偏差掩盖 spec_gap_masking 测试层变体
