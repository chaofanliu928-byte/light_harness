...
| 7 | cross-ref hook PAIRS 第 6 条:删 design-rules.md 内 `**轻量级**` 字面 → hook 报警 | meta-L1 | check-meta-cross-ref exit 2 + 列具体缺失 |
...
25. **多次审查错误判断过程留痕**(`feedback_spec_gap_masking` 元数据点):
   ...
   - **教训**:hook 实现 trial(P0.9.3 第一个 + 本 trial)涉及大量字面 grep + anchor 比对工作,审查者 + 调度者 + 自审 + spec 写者都容易**视觉跳过间接引用 / 不验证 anchor 字面存在性 / 修复时只 sweep 命中点不全文扫**;后续 trial 涉及 anchor / PAIRS 比对时必须:
     ...
     4. **修字面错时全文 grep 扫所有 stale references**(spec + plan + decision + commit msg template),而非只改命中点 — 否则 stale ref 会通过 final reviewer 暴露(第五次错教训)
   - 留痕作为 P0.9.3 trial 序列的元数据点(hook 实现 trial 的高 finding 密度部分来自这种"视觉跳过 / 字面未验证 / 修复 sweep 不全"模式)
