...
25. **多次审查错误判断过程留痕**(`feedback_spec_gap_masking` 元数据点):
   ...
   - **教训**:hook 实现 trial(P0.9.3 第一个 + 本 trial)涉及大量字面 grep + anchor 比对工作,审查者 + 调度者 + 自审 + spec 写者都容易**视觉跳过间接引用 / 不验证 anchor 字面存在性**;后续 trial 涉及 anchor / PAIRS 比对时必须:
   ...
   - 留痕作为 P0.9.3 trial 序列的元数据点(hook 实现 trial 的高 finding 密度部分来自这种"视觉跳过 / 字面未验证"模式)
